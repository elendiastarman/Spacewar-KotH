function FutureRammer_setup(team) {
	var botVars = {};
	
	botVars["color"] = team;
	botVars["enemy"] = team === "red" ? "blue" : "red";
	
	botVars["forecast"] = function(ownColor, enemyColor, action, iterations, gameInfo) {
		var ship = {'x':gameInfo[ownColor+"_x"],'y':gameInfo[ownColor+"_y"],'xv':gameInfo[ownColor+"_xv"],'yv':gameInfo[ownColor+"_yv"]};
		var enemy = {'x':gameInfo[enemyColor+"_x"],'y':gameInfo[enemyColor+"_y"],'xv':gameInfo[enemyColor+"_xv"],'yv':gameInfo[enemyColor+"_yv"]};
		var sx = gameInfo["sun_x"];
		var sy = gameInfo["sun_y"];
	}
	
	return botVars;
}

function FutureRammer_getActions(gameInfo, botVars) {
	var actions = [];
	...
	if (gameInfo[botVars["color"]+"_alive"] && gameInfo[botVars["enemy"]+"_alive"]) {
		...
		// var ang2 = Math.degrees( Math.atan2(dy1/dis1 - dy2/dis2, dx1/dis1 - dy2/dis2) );
		var ang2 = Math.degrees( Math.atan2(dy1 + dy2, dx1 + dy2) );
